...
class ApplyUnifiedPatchSessionRuntimeTests(unittest.TestCase):
    def test_apply_unified_patch_is_blocked_without_fresh_full_read_grounding(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manager = SessionManager(store=store, backend=_UnusedBackend(), workspace_root=str(workspace), enable_mcp_filesystem=True)
    ...
    def test_apply_unified_patch_executes_after_fresh_full_read_grounding(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            read_request = ToolRequest(tool_name="read_file", input_payload={"path": "notes.txt"}, requires_approval=False, side_effect_class="safe")
